...
export const getQueryParams = (params: ProductsQueryParamsSchemaType) => {
  ...
  try {
    ...
    if (categories && Array.isArray(categories) && categories[0] !== 0) query.category = {$in: categories};
    ...
    if (search) {
      const options = {$regex: '\\b' + search, $options: 'i'};
      query.$or = [
        {title: options},
        {brand: options},
  ...
  return {query, skip, pageIdx, itemsLimit, categories};
};
